...
const userUpdateRoute = async (req, res) => {
    ...
    try {
        ...
        if (user) {
            return res.status(201).send({ Message: `User_id ${user._id} Updated Success` });
        } else {
...
const userDeleteRoute = async (req, res) => {
    try {
        ...
        if (user) {
            return res.status(201).send({ Message: `User_id ${user._id} Deleted Success` });
        } else {
            return res.status(404).send({ Message: "User is not Exist" });
        }
    } catch (error) {
...
module.exports = { userCreateRoute, userUpdateRoute, userDeleteRoute, userGetAllRoute, userGetByIdRoute };